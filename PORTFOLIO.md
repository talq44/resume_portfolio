...
gantt
  ...
  MVVM :a1, 2020-05, 2025-10
  TMA: a5, 2024-01, 2025-10
  MVI :a2, 2022-07, 2025-10
  Clean Architecture :a1, 2022-01, 2025-10
  VIPER: a4, 2020-06, 2022-06
  Ribs: a4, 2020-06, 2022-09
  MVC : a3, 2015-05, 2020-12

  section UI
  SwiftUI : c1, 2023-07, 2025-10
  PinLayout/FlexLayout : u3, 2022-03, 2022-07
  SnapKit : c3, 2020-01, 2025-10
  AutoLayout : c2, 2016-07, 2025-10
  Autoresizing Mask : u2, 2015-09, 2017-03
  StoryBoard : u1, 2015-09, 2023-01

  section Asynchronous
  Async/Await: r3, 2022-07, 2025-10
  Combine: r2, 2020-02, 2025-10
  RxSwift: r1, 2019-07, 2025-10
  ...
  FireBase(GoogleAnayltics) : s2, 2016-12, 2025-10

  section Tools
  Xcode-cloud: t6, 2022-06, 2025-10
  Github-Action: t6, 2022-01, 2025-10
  Slack: t5, 2021-07, 2025-10
  Jira: t4, 2021-02, 2024-12
  Figma: t3, 2020-07, 2025-10
  Zeplin: t2, 2018-07, 2025-01
  Git: t1, 2016-06, 2025-10
...
### 주요 기술
- Swift, .xcconfig, target, CI/CD

### 도전 과제 및 해결 방법
- 하나의 소스로 두개 이상의 서비스를 제공할 수 있도록 구조 확립
  - 일정 관계상 target을 추가하는 방식을 설정
  - xcconfig를 활용해 환경변수화 할 수 있는 값들을 분리
  - 디자인시스템 구축으로 Theme별 Color를 제공할 수 있는 구조로 변경
  - Asset 관리를 위한 구조 설정
- 서로 다른 서비스를 잦게 확인하기 위한 CI/CD 추가
  - PR시 서로 다른 타깃 모두 build를 확인할 수 있도록 파이프라인 구성
  - 별도 리소스를 할당 받을 수 없어 Xcode Cloud를 통해 구성
  - 특정 브랜치에 merge시 배포 자동화

### 주요 성과
- 2달 -> 1달반으로 줄어든 일정내에서도 정상적 배포
- CI/CD 도입을 통한 배포 리소스 효율화
  - 배포를 위한 추가 리소스를 투입하지 않도록 개선
- 버전 및 다수의 환경변수를 공통화

### 배운점/아쉬운점
- 점진적 모듈화를 도입하지 못한 아쉬움
  - 전반적인 QA가 들어가는 만큼 구조적 개선을 더 진행하지 못한데에 대한 아쉬움
- 초기형 디자인시스템 모듈 도입
  - atomic한 정의까지 진행해 그 이후의 개선을 더 하지못한데에 대한 아쉬움
  - 컴포넌트 단위까지 점진적으로 적용 및 대체 진행 중
- Target을 추가하면 C,D,E... 앱 추가 가능한 구조 확립
  - 앞으로 추가 가능성이 있는 구조에 대해 적용이 가능해지도록 함
...
## 아몬즈 리소스 및 프레임워크 최적화를 통한 용량 개선

### 개요
- 목표: 아몬즈 앱 용량 개선을 위한 리소스 수정 및 프레임워크 개선
- 기간: 2024.06 ~ 2024.07
- 규모: iOS 2, QA 1
- 역할: iOS 개발 및 리딩

### 주요 기술
- [Tuist](https://docs.tuist.dev/en/), [The Modular Architecture(TMA)](https://docs.tuist.dev/en/guides/develop/projects/tma-architecture)
- Design System

### 도전 과제 및 해결 방법
- 폰트 삭제를 통한 용량 개선
  - 사용하지 않는 폰트 삭제
  - 사용량이 적은 폰트 Design System을 토대로 대체
- 사용하지 않는 이미지 삭제를 통한 용량 개선
- 프레임워크 추가 개선
...
### 배운점/아쉬운점
- 디자인팀과 디자인시스템 구축 및 지속적 리소스 삭제를 위한 내용 공유
- Static <-> Dynamic 의 차이에 대한 파악
- Static의 중복 적용으로 인한 메모리 향상을 방지하기 위해 TMA의 적극적 적용
- 용량 감소시 업데이트율이 오른다는 내용은 맞지 않았음을 확인
  - 사용자들의 자동 업데이트에 대한 비율 재 확인
...
## 성능 모니터링 도입 및 개선
### 개요
- 목표: App팀 성능 개선을 위한 모니터링 추가
- 기간: 2023.05 ~ 2023.06
- 규모: iOS 1, Android 1, PO 1, QA 1, Back 1
- 역할: 모니터링 체계 리딩 및 iOS 개발

### 주요 기술
- Firebase (Performance, Crasylytics), BigQuery

### 도전 과제 및 해결 방법
- Android, iOS 앱의 성능 파악의 필요성 대두
  - Sentry, DataDog, Firebase 등 다양한 방식에 대한 확인 진행
  - Firebase를 사용중이었기 때문에, Firebase Performance 적용하기로 결정
- 주요 파악 지표
  - UI 노출 속도
  - 네트워크 속도
  - 크래쉬 율
...
### 주요 성과
- 화면 랜더링이 늦었던 5개 페이지 파악 및 정상화
- 중복 요청 API의 80% 제거
  - 이후 100% 제거 및 지속적 모니터링
- 응답속도가 0.3ms 이상 걸리는 API 100% 파악 및 속도 향상
  - 이후 0.3ms 이상의 API path 공유 프로세스 확립
- 크래쉬 발생 공유 및 주단위 크래쉬 비율 공유 프로세스 확립
  - slack을 통한 크래쉬 발생시 공유 및 Jira ticket 생성 자동화
  - 매주 월요일 지난 1주동안 있었던 크래쉬 비율과 유저 수에 대한 공유 자동화

### 배운점/아쉬운점
- 파악이 된다고 해도 지속적인 공유(ex Slack)를 통해서 개선이 가능하다는것을 확인
- 숨기지 않고 공유했을때, 더 빠른 대처와 노력을 하게됨을 확인
- 정확한 데이터를 통해 대화할때 유관부서와 더 빠른 대응이 가능함을 확인
- 툴의 도입 뿐만 아니라 체계와 공유의 중요성 확인
- Firebase 이외의 툴을 통해 성능개선을 이어가지 못한 아쉬움
...
- 테스트커버리지 15% 향상
- 버그 발생율 30% 감소
- Analytics 개선 작업시 작업속도 타 플랫폼 대비 40% 빠르게 작업
...
- 함수로 정의되어있던 부분을 Input/Output 패턴적용
